...
export function LinkCard({ type, title, icon, link, delay }: LinkProps) {
  return (
    <Link
      href={link.toString()}
      target={`${type == "link" ? "_blank" : ""}`}
      className={`relative bg-zinc-500 bg-opacity-20 flex items-center justify-center py-5 rounded-xl ring ring-zinc-500 px-[2rem] md:ring-0 md:bg-opacity-10 md:hover:ring transition-all group duration-500 origin-top animate-show-link `}
